...
export async function generateStaticParams() {
    console.log('开始生成静态参数...');
    try {
        const posts = await getPosts();
        console.log(`找到${posts.length}篇文章用于静态生成`);
...
export default async function PostPage({ params }: { params: { slug: string } }) {
    ...
    try {
            ...
            return (
                <div className="min-h-screen flex items-center justify-center bg-gray-50">
                    <div className="text-center">
                        ...
                        <p className="text-xl text-gray-600 mb-4">文章不存在</p>
                        <p className="text-gray-500">请求的文章: {params.slug}</p>
                    </div>
                </div>
            );
        }
        ...
        console.error('渲染文章页面时发生错误:', error);
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    <h1 className="text-6xl font-bold text-gray-900 mb-4">500</h1>
                    <p className="text-xl text-gray-600">服务器错误</p>
                </div>
            </div>
        );
    }
